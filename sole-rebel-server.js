...
const VISITOR_BOT_PATTERN=/bot|crawl|spider|slurp|preview|facebookexternalhit|whatsapp|uptime|monitor|headless/i;

app.use(express.json());
app.use((req,_res,next)=>{
  if(req.method==='GET'&&req.path==='/'&&!validSession(req)&&!VISITOR_BOT_PATTERN.test(String(req.headers['user-agent']||''))){
    void recordStorefrontVisit(req).catch(err=>console.error('Sole Rebel visitor tracking error:',err.message));
  }
  next();
});
...
async function ensureAnalyticsSchema(){
  ...
  await pool.query('CREATE INDEX IF NOT EXISTS idx_sole_rebel_visits_visited_at ON sole_rebel_visits(visited_at)');
  await pool.query('CREATE INDEX IF NOT EXISTS idx_sole_rebel_visits_visitor_key ON sole_rebel_visits(visitor_key)');
}

async function recordStorefrontVisit(req){
  await ensureAnalyticsSchema();
  const key=visitorKey(req);
  await pool.query(`INSERT INTO sole_rebel_visits(visitor_key,visited_at)
    SELECT $1,NOW()
    WHERE NOT EXISTS (
...
app.get('/api/admin/dashboard',requireAdmin,async(_req,res)=>{try{const {businessId}=await ids();await ensureAnalyticsSchema();const [stats,orders,visitors]=await Promise.all([pool.query(`SELECT COUNT(*) FILTER (WHERE ordered_at>=NOW()-INTERVAL '24 hours')::int AS last24h,COUNT(*) FILTER (WHERE ordered_at>=NOW()-INTERVAL '7 days')::int AS last7d,COUNT(*)::int AS lifetime,COUNT(*) FILTER (WHERE status='paid')::int AS paid,COUNT(*) FILTER (WHERE status='payment_reported')::int AS payment_reported,COUNT(*) FILTER (WHERE status NOT IN ('paid','cancelled','refunded'))::int AS unpaid,COALESCE(SUM(total),0)::numeric AS order_value,COALESCE(SUM(total) FILTER (WHERE status='paid'),0)::numeric AS paid_revenue FROM orders WHERE business_id=$1`,[businessId]),pool.query(`SELECT id,external_order_id,friendly_order_number,status,total,ordered_at,fulfillment_status,metadata FROM orders WHERE business_id=$1 ORDER BY ordered_at DESC LIMIT 200`,[businessId]),pool.query(`SELECT COUNT(DISTINCT visitor_key) FILTER (WHERE visited_at>=NOW()-INTERVAL '24 hours')::int AS visitors_24h,COUNT(DISTINCT visitor_key) FILTER (WHERE visited_at>=NOW()-INTERVAL '7 days')::int AS visitors_7d,COUNT(DISTINCT visitor_key)::int AS visitors_lifetime FROM sole_rebel_visits`)]);res.json({stats:{...stats.rows[0],...visitors.rows[0],order_value:Number(stats.rows[0].order_value),paid_revenue:Number(stats.rows[0].paid_revenue)},orders:orders.rows.map(o=>({...o,total:Number(o.total)}))});}catch(e){res.status(500).json({error:e.message});}});
...
registerStampsShippingRoutes(app,{pool,requireAdmin,trackingUrl,sessionSecret:SESSION_SECRET});

app.get('/dashboard',async(_req,res)=>{try{const {readFile}=await import('fs/promises');const file=path.join(__dirname,'public','sole-rebel','dashboard.html');const html=await readFile(file,'utf8');res.type('html').send(html.replace('</head>','<link rel="stylesheet" href="/stamps-shipping-admin.css"></head>').replace('</body>','<script src="/order-delete.js"></script><script src="/visitor-analytics.js"></script><script src="/stamps-shipping-admin.js"></script></body>'));}catch(e){res.status(500).send('Unable to load Sole Rebel dashboard.');}});
