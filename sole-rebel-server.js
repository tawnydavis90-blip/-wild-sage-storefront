...
app.get('/dashboard',async(_req,res)=>{try{const {readFile}=await import('fs/promises');const file=path.join(__dirname,'public','sole-rebel','dashboard.html');const html=await readFile(file,'utf8');res.type('html').send(html.replace('</body>','<script src="/order-delete.js"></script><script src="/visitor-analytics.js"></script></body>'));}catch(e){res.status(500).send('Unable to load Sole Rebel dashboard.');}});
