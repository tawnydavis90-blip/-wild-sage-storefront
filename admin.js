...
async function stripeOrders(){if(!stripe)return{configured:false,orders:[]};const sessions=await stripe.checkout.sessions.list({limit:50,expand:['data.line_items','data.payment_intent']});return{configured:true,orders:sessions.data.map(s=>{const state=checkoutState(s);return{id:s.id,created:new Date(s.created*1000).toISOString(),status:s.payment_status,checkoutState:state,isActualOrder:s.payment_status==='paid',total:Number(s.amount_total||0)/100,currency:String(s.currency||'usd').toUpperCase(),customer:s.customer_details?.email||s.customer_email||'',name:s.customer_details?.name||'',items:(s.line_items?.data||[]).map(i=>({description:i.description,quantity:i.quantity,amount:Number(i.amount_total||0)/100}))};})};}
