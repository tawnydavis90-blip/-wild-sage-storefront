...
async function readOverrides(){if(!(await ensureSchema()))return{configured:false,items:[]};const{rows}=await getPool().query('SELECT product_id,featured,best_seller,current_drop,mockup_urls,updated_at FROM product_merchandising ORDER BY updated_at DESC');return{configured:true,items:rows.map(r=>({productId:r.product_id,featured:Boolean(r.featured),bestSeller:Boolean(r.best_seller),currentDrop:Boolean(r.current_drop),mockups:Array.isArray(r.mockup_urls)?r.mockup_urls:[],updatedAt:r.updated_at}))};}
...
export function registerAdminRoutes(app){
  ...
  app.put('/api/admin/products/:id',requireAdmin,async(req,res)=>{if(!(await ensureSchema()))return res.status(503).json({error:'DATABASE_URL is not configured.'});const productId=cleanText(req.params.id,140);if(!productId)return res.status(400).json({error:'Product id is required.'});const currentDrop=Boolean(req.body?.currentDrop),featured=Boolean(req.body?.featured),bestSeller=Boolean(req.body?.bestSeller),mockups=sanitizeMockups(req.body?.mockups);try{const{rows}=await getPool().query(`INSERT INTO product_merchandising(product_id,current_drop,featured,best_seller,mockup_urls,updated_at) VALUES($1,$2,$3,$4,$5::jsonb,NOW()) ON CONFLICT(product_id) DO UPDATE SET current_drop=EXCLUDED.current_drop,featured=EXCLUDED.featured,best_seller=EXCLUDED.best_seller,mockup_urls=EXCLUDED.mockup_urls,updated_at=NOW() RETURNING product_id,current_drop,featured,best_seller,mockup_urls,updated_at`,[productId,currentDrop,featured,bestSeller,JSON.stringify(mockups)]);const r=rows[0];res.json({ok:true,item:{productId:r.product_id,currentDrop:r.current_drop,featured:r.featured,bestSeller:r.best_seller,mockups:r.mockup_urls,updatedAt:r.updated_at}})}catch(err){console.error(err);res.status(500).json({error:'Unable to save product settings.'})}});
}