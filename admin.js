...
import { hiddenOrderIds } from './order-visibility.js';
...
async function ensureSchema(){
  ...
  await db.query(`
    ...
    ALTER TABLE product_merchandising ADD COLUMN IF NOT EXISTS current_drop BOOLEAN NOT NULL DEFAULT FALSE;
    ALTER TABLE product_merchandising ADD COLUMN IF NOT EXISTS mockup_map JSONB NOT NULL DEFAULT '{}'::jsonb;
...
function validSession(req){if(!secret())return false;const token=parseCookies(req)[COOKIE_NAME];if(!token)return false;const parts=token.split('.');if(parts.length!==3)return false;const payload=`${parts[0]}.${parts[1]}`,expected=sign(payload),actual=parts[2];if(expected.length!==actual.length||!crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(actual)))return false;return Number(parts[0])>Date.now();}
function requireAdmin(req,res,next){if(!validSession(req))return res.status(401).json({error:'Admin login required.'});next();}
function requireParentDashboard(req,res,next){const expected=String(process.env.PARENT_DASHBOARD_API_KEY||'');if(!expected)return res.status(503).json({error:'Parent dashboard integration is not configured.'});const auth=String(req.headers.authorization||''),supplied=auth.startsWith('Bearer ')?auth.slice(7):'';const ok=supplied.length===expected.length&&supplied.length>0&&crypto.timingSafeEqual(Buffer.from(supplied),Buffer.from(expected));if(!ok)return res.status(401).json({error:'Invalid parent dashboard credentials.'});next();}
function sanitizeMockups(value){if(!Array.isArray(value))return[];return value.map(v=>String(v||'').trim()).filter(v=>/^https?:\/\//i.test(v)).slice(0,12);}
function sanitizeMockupMap(value){const out={main:{front:'',back:''},colors:{}};if(!value||typeof value!=='object')return out;const url=v=>{const s=String(v||'').trim();return /^https?:\/\//i.test(s)?s:''};out.main.front=url(value.main?.front);out.main.back=url(value.main?.back);if(value.colors&&typeof value.colors==='object'){for(const [color,row] of Object.entries(value.colors).slice(0,40)){const key=cleanText(color,100);if(!key)continue;const front=url(row?.front),back=url(row?.back);if(front||back)out.colors[key]={front,back};}}return out;}
function cleanText(value,max=1000){return String(value??'').trim().slice(0,max);}
async function readOverrides(){if(!(await ensureSchema()))return{configured:false,items:[]};const{rows}=await getPool().query('SELECT product_id,featured,best_seller,current_drop,mockup_urls,mockup_map,updated_at FROM product_merchandising ORDER BY updated_at DESC');return{configured:true,items:rows.map(r=>({productId:r.product_id,featured:Boolean(r.featured),bestSeller:Boolean(r.best_seller),currentDrop:Boolean(r.current_drop),mockups:Array.isArray(r.mockup_urls)?r.mockup_urls:[],mockupMap:r.mockup_map&&typeof r.mockup_map==='object'?r.mockup_map:{main:{front:'',back:''},colors:{}},updatedAt:r.updated_at}))};}
async function readSettings(){const result={...DEFAULT_SETTINGS};if(!(await ensureSchema()))return{configured:false,settings:result,collections:DEFAULT_COLLECTIONS.map(([id,label,enabled,sortOrder])=>({id,label,enabled,sortOrder}))};const db=getPool();const settings=await db.query('SELECT setting_key,setting_value FROM store_settings');settings.rows.forEach(r=>{if(Object.prototype.hasOwnProperty.call(result,r.setting_key))result[r.setting_key]=r.setting_value;});const collections=await db.query('SELECT collection_id,label,enabled,sort_order FROM collection_settings ORDER BY sort_order,label');return{configured:true,settings:result,collections:collections.rows.map(r=>({id:r.collection_id,label:r.label,enabled:r.enabled,sortOrder:r.sort_order}))};}
async function analyticsSummary(){if(!(await ensureSchema()))return{configured:false,windows:{}};const db=getPool();const windows={};for(const [key,interval] of [['24h','24 hours'],['7d','7 days'],['30d','30 days'],['all',null]]){const where=interval?`WHERE created_at >= NOW() - INTERVAL '${interval}'`:'';const q=await db.query(`SELECT event_type,COUNT(*)::int AS count,COUNT(DISTINCT session_id)::int AS sessions FROM analytics_events ${where} GROUP BY event_type`);const bucket={pageViews:0,productViews:0,addToBags:0,checkoutStarts:0,uniqueSessions:0};q.rows.forEach(r=>{if(r.event_type==='page_view')bucket.pageViews=r.count;if(r.event_type==='product_view')bucket.productViews=r.count;if(r.event_type==='add_to_bag')bucket.addToBags=r.count;if(r.event_type==='checkout_start')bucket.checkoutStarts=r.count;bucket.uniqueSessions=Math.max(bucket.uniqueSessions,r.sessions||0);});windows[key]=bucket;}const top=await db.query(`SELECT product_id,COUNT(*)::int AS views FROM analytics_events WHERE event_type='product_view' AND product_id IS NOT NULL AND created_at>=NOW()-INTERVAL '30 days' GROUP BY product_id ORDER BY views DESC LIMIT 10`);return{configured:true,windows,topProducts:top.rows.map(r=>({productId:r.product_id,views:r.views}))};}
function checkoutState(s){if(s.payment_status==='paid')return'paid';if(s.status==='expired'&&!s.payment_intent)return'abandoned';if(s.payment_intent)return'payment_not_completed';return String(s.status||'unpaid');}
async function stripeOrders(){if(!stripe)return{configured:false,orders:[]};const[sessions,hidden]=await Promise.all([stripe.checkout.sessions.list({limit:50,expand:['data.line_items','data.payment_intent']}),hiddenOrderIds()]);return{configured:true,orders:sessions.data.filter(s=>!hidden.has(String(s.id))).map(s=>{const state=checkoutState(s);return{id:s.id,created:new Date(s.created*1000).toISOString(),status:s.payment_status,checkoutState:state,isActualOrder:s.payment_status==='paid',total:Number(s.amount_total||0)/100,currency:String(s.currency||'usd').toUpperCase(),customer:s.customer_details?.email||s.customer_email||'',name:s.customer_details?.name||'',items:(s.line_items?.data||[]).map(i=>({description:i.description,quantity:i.quantity,amount:Number(i.amount_total||0)/100}))};})};}
async function parentSummary(){const merch=await readOverrides(),analytics=await analyticsSummary(),settings=await readSettings();const items=merch.items||[];return{business:{id:BUSINESS_ID,name:BUSINESS_NAME,parentCompanyId:PARENT_COMPANY_ID,service:'storefront',version:2},databaseConfigured:merch.configured,merchandising:{configuredProducts:items.length,currentDropProducts:items.filter(x=>x.currentDrop).length,featuredProducts:items.filter(x=>x.featured).length,bestSellerProducts:items.filter(x=>x.bestSeller).length,customMockupProducts:items.filter(x=>x.mockups?.length||x.mockupMap?.main?.front||Object.keys(x.mockupMap?.colors||{}).length).length},analytics:analytics.windows?.['30d']||{},store:{featuredEnabled:Boolean(settings.settings?.featuredEnabled),bestSellersEnabled:Boolean(settings.settings?.bestSellersEnabled)},capabilities:['merchandising.read','merchandising.write','mockups.read','mockups.write','orders.read','analytics.read','homepage.read','homepage.write','collections.read','collections.write','settings.read','settings.write']};}
...
export function registerAdminRoutes(app){
  ...
  app.put('/api/admin/products/:id',requireAdmin,async(req,res)=>{if(!(await ensureSchema()))return res.status(503).json({error:'DATABASE_URL is not configured.'});const productId=cleanText(req.params.id,140);if(!productId)return res.status(400).json({error:'Product id is required.'});const currentDrop=Boolean(req.body?.currentDrop),featured=Boolean(req.body?.featured),bestSeller=Boolean(req.body?.bestSeller),mockups=sanitizeMockups(req.body?.mockups),mockupMap=sanitizeMockupMap(req.body?.mockupMap);try{const{rows}=await getPool().query(`INSERT INTO product_merchandising(product_id,current_drop,featured,best_seller,mockup_urls,mockup_map,updated_at) VALUES($1,$2,$3,$4,$5::jsonb,$6::jsonb,NOW()) ON CONFLICT(product_id) DO UPDATE SET current_drop=EXCLUDED.current_drop,featured=EXCLUDED.featured,best_seller=EXCLUDED.best_seller,mockup_urls=EXCLUDED.mockup_urls,mockup_map=EXCLUDED.mockup_map,updated_at=NOW() RETURNING product_id,current_drop,featured,best_seller,mockup_urls,mockup_map,updated_at`,[productId,currentDrop,featured,bestSeller,JSON.stringify(mockups),JSON.stringify(mockupMap)]);const r=rows[0];res.json({ok:true,item:{productId:r.product_id,currentDrop:r.current_drop,featured:r.featured,bestSeller:r.best_seller,mockups:r.mockup_urls,mockupMap:r.mockup_map,updatedAt:r.updated_at}})}catch(err){console.error(err);res.status(500).json({error:'Unable to save product settings.'})}});
}
