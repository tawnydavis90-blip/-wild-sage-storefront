...
export function registerOrderDetailRoutes(app){
  app.get('/api/admin/orders/:id',requireAdmin,async(req,res)=>{
    ...
    const isTest=id.startsWith('cs_test_'),client=isTest?testStripe:stripe;
    if(!client) return res.status(503).json({error:isTest?'Stripe test mode is not configured on this service.':'Stripe is not configured on this service.'});
    try{
      const [session, fulfillmentResult] = await Promise.all([
        client.checkout.sessions.retrieve(id,{expand:['line_items.data.price.product','payment_intent','customer']}),
        !isTest&&printifyConfigured()
      ...
      res.json({
        id:session.id,isTest,
        created:new Date(session.created*1000).toISOString(),
        expiresAt:session.expires_at?new Date(session.expires_at*1000).toISOString():null,
        paymentStatus:session.payment_status||'',status:session.status||'',mode:session.mode||'',
        currency:String(session.currency||'usd').toUpperCase(),
        amountSubtotal:Number(session.amount_subtotal||0)/100,amountTotal:Number(session.amount_total||0)/100,
        shippingAmount:Number(session.total_details?.amount_shipping||0)/100,taxAmount:Number(session.total_details?.amount_tax||0)/100,discountAmount:Number(session.total_details?.amount_discount||0)/100,
        customer:{name:customer.name||'',email:customer.email||session.customer_email||'',phone:customer.phone||'',address:address(customer.address)},
        shipping:{name:shipping?.name||'',address:address(shipping?.address)},
        payment:{intentId:intent?.id||String(session.payment_intent||''),status:intent?.status||session.payment_status||'',paymentMethodTypes:intent?.payment_method_types||session.payment_method_types||[],receiptEmail:intent?.receipt_email||''},
        clientReferenceId:session.client_reference_id||'',metadata:session.metadata||{},items,
        fulfillment:{configured:isTest?false:printifyConfigured(),matched:Boolean(fulfillmentResult),testBlocked:isTest,order:fulfillmentResult}
      });
    }catch(err){console.error('Stripe order detail error:',err);res.status(err?.statusCode===404?404:500).json({error:'Unable to load order details.'});}
  });
}
