...
async function stripeWildSageSummary(){
  if(!stripe)return null;
  try{
    const sessions=await stripe.checkout.sessions.list({limit:100});
    const paid=sessions.data.filter(s=>s.payment_status==='paid'&&s.metadata?.wild_sage_test!=='true'&&s.metadata?.do_not_fulfill!=='true'&&!String(s.id||'').startsWith('cs_test_'));
    return {orders:paid.length,revenue:paid.reduce((sum,s)=>sum+Number(s.amount_total||0)/100,0)};
  }catch(err){console.error('Sage & Ember Stripe summary error:',err.message);return null;}
}
...
async function companySummary(){
  ...
  const live=await stripeWildSageSummary();
  if(live){const wild=businesses.find(b=>b.slug==='wild-sage-apparel');if(wild){wild.orders=live.orders;wild.revenue=live.revenue;wild.net=wild.revenue-wild.expenses;wild.salesSource='stripe';}}
  return businesses;
}
...
export function registerParentDashboardRoutes(app){
  app.get('/api/holding/health',requireParentKey,async(_req,res)=>{try{const result=await db().query('SELECT NOW() AS now');res.json({ok:true,database:true,stripe:Boolean(stripe),at:result.rows[0].now});}catch(err){res.status(500).json({ok:false,error:err.message});}});
