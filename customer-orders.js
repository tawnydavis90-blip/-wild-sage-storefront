...
async function fulfillment(sessionId){if(String(sessionId).startsWith('cs_test_'))return{test:true,status:'test order — not sent to Printify',tracking:[]};if(String(process.env.LIVE_FULFILLMENT_DRY_RUN||'').toLowerCase()==='true')return{dryRun:true,status:'payment received — fulfillment test mode',tracking:[]};try{const orders=await listPrintifyOrders(20),o=orders.find(x=>printifyExternalId(x)===String(sessionId));if(!o)return null;const shipments=Array.isArray(o.shipments)?o.shipments:[];return{id:o.id,status:o.status||'',tracking:shipments.map(s=>({carrier:s.carrier||'',number:s.number||s.tracking_number||'',url:s.url||s.tracking_url||'',deliveredAt:s.delivered_at||null})).filter(x=>x.number||x.url)};}catch{return null;}}
