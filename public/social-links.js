(()=>{
  ...
  async function start(){
    ...
    const wrap=document.createElement('section');wrap.className='ws-social-links';wrap.innerHTML=`<p class="ws-social-kicker">FOLLOW THE WILD</p><div class="ws-social-row">${order.filter(key=>String(links[key]||'').trim()).map(key=>{const url=String(links[key]).trim();return `<a class="ws-social-button" href="${url.replace(/"/g,'&quot;')}" target="_blank" rel="noopener noreferrer" aria-label="${labels[key]}"><span>${icons[key]}</span><strong>${labels[key]}</strong></a>`}).join('')}</div>`;
