(()=>{
  ...
  const labels={instagram:'Instagram',facebook:'Facebook',tiktok:'TikTok',pinterest:'Pinterest'};
  const order=['instagram','facebook','tiktok','pinterest'];
  async function start(){
    let links={instagram:'',facebook:'',tiktok:'',pinterest:''};
    try{const r=await fetch('/api/social-links',{headers:{Accept:'application/json'}});if(r.ok){const d=await r.json();links={...links,...(d.links||{})}}}catch(e){console.warn('Social links unavailable',e)}
    const wrap=document.createElement('section');
    wrap.className='ws-social-links';
    wrap.innerHTML=`<p class="ws-social-kicker">FOLLOW THE WILD</p><div class="ws-social-row">${order.map(key=>{const url=links[key]||'';return url?`<a class="ws-social-button" href="${String(url).replace(/"/g,'&quot;')}" target="_blank" rel="noopener noreferrer" aria-label="${labels[key]}"><span>${icons[key]}</span><strong>${labels[key]}</strong></a>`:`<div class="ws-social-button is-unlinked" aria-label="${labels[key]} not connected yet"><span>${icons[key]}</span><strong>${labels[key]}</strong></div>`}).join('')}</div>`;
