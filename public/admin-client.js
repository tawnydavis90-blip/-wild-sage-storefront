(() => {
  ...
  function fillConfig(){const s=storeConfig?.settings||{};['announcementLeft','announcementRight','heroEyebrow','heroTitle','heroKicker','aboutEyebrow','aboutTitle','aboutText','contactEmail'].forEach(k=>{const el=$('#'+k);if(el)el.value=s[k]??'';});$('#featuredEnabled').checked=Boolean(s.featuredEnabled);$('#bestSellersEnabled').checked=Boolean(s.bestSellersEnabled);$('#collectionsList').innerHTML=(storeConfig?.collections||[]).map(c=>`<div class="collection-row" data-id="${esc(c.id)}"><input class="collection-enabled" type="checkbox" ${c.enabled?'checked':''} aria-label="Enable ${esc(c.label)}"><input class="collection-label" value="${esc(c.label)}"><input class="sort-order" type="number" value="${Number(c.sortOrder||0)}" aria-label="Sort order"></div>`).join('');}
  async function loadConfig(){storeConfig=await json('/api/admin/store-config');fillConfig();}
